...
tab = data.table(score = dat_dt_z$Smad4, ident = dat_dt_z$ident) %>% 
  mutate(grouping = ifelse(ident %in% levels(mglia)[c(1:5,8)], "Homeostatic",
                           ifelse(ident %in% levels(mglia)[9:10], "DAMs",
                                  ifelse(ident %in% levels(mglia)[16:18], "TIMs",
                                         ifelse(ident %in% levels(mglia)[11:15], "Inflammatory","Other Microglia"))))) %>%
  mutate(grouping = factor(grouping, levels = c("Homeostatic","Inflammatory","TIMs","Other Microglia","DAMs")))

stat.test = tab %>% wilcox_test(score ~ grouping) %>% add_significance() %>% add_xy_position(x = "grouping")
stat.test.dams = stat.test %>% 
  filter(group1 == "DAMs" | group2 == "DAMs") %>% 
  mutate(y.position = c(3.5,5,6.5,8))

(ggplot(tab, aes(x=grouping,y=score)) + 
    geom_boxplot(aes(fill=grouping),outlier.shape = NA, size = 1.25) + 
    stat_pvalue_manual(stat.test.dams, label = "p = {p.adj}", vjust = -0.25, size = 10, bracket.size = 1.5) +
    ylim(-3,9) +
    theme_bw() + 
    theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1, size = 25), plot.margin = margin(0.1,0.1,0.1,1.2, "cm"),
          axis.title = element_text(size=30), axis.text.y = element_text(size=25)) +
...
dat_sums$group = factor(dat_sums$group, levels = c("Homeostatic","Inflammatory","TIMs","Other Microglia","DAMs","Non-Microglia"))
(ggplot(dat_sums, aes(x=group,y=mean,fill=group)) + 
    geom_bar(stat = "identity", color = "black", linewidth = 1) + 
    geom_errorbar(aes(ymin=mean-sem,ymax=mean+sem),width=0.2, linewidth=1,position=position_dodge(0.9)) +
    theme_bw() + 
    labs(y = "Total Interactions", x= "", fill = "Group") +
    theme(axis.title = element_text(size=30), axis.text.y = element_text(size=25), 
          axis.text.x = element_text(angle = 45, hjust = 1, size = 25)) +
    NoLegend()
) %>% ggsave("interactions_barplot.png",.,height=12,width=12,dpi=600)
